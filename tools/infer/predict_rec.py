...
class TextRecognizer(object):
    ...
    def resize_norm_img(self, img, max_wh_ratio):
        ...
        resized_image = cv2.resize(img, (int(resized_w), int(imgH)))
        resized_image = resized_image.astype('float32')
        resized_image = resized_image.transpose((2, 0, 1)) / 255
        resized_image -= 0.5
        resized_image /= 0.5
        ...
        if resized_w > 320:
            ...
            resized_image = cv2.resize(img, (int(320), int(32)))
            resized_image = resized_image.astype('float32')
            resized_image = resized_image.transpose((2, 0, 1)) / 255
            resized_image -= 0.5
            resized_image /= 0.5
            ...
            padding_im[:, :, 0:int(resized_w)] = resized_image
            return padding_im
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        if len(img_list) >= args.rec_batch_num or idx == len(
            ...
            with open('result.txt','a+',encoding='utf-8') as f:
                ...
                #     ino], rec_res[ino]))
                    line = valid_image_file_list[ino].split('/')[-1] + '\t' + rec_res[ino][0] + '\n'
                    print(line)
                    f.write(line)
